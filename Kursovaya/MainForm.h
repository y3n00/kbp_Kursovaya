...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void) {
			...
			this->dateTimePicker1->MinDate = DateTime::Now.AddMinutes(30);
...
#pragma endregion
	private:
		...
		void saveTable(DataGridView^ from, nlohmann::json& to) {
			const auto DG = from;
			const auto cols = DG->ColumnCount, rows = DG->RowCount;
			for(int i = 0; i < rows; ++i) {
				auto& entry = to["Data"][i];
				const auto cells = DG->Rows[i]->Cells;
				entry["Date"] = string_cast(cells[0]->Value->ToString());
				entry["Name"] = string_cast(cells[1]->Value->ToString());
				entry["Services"] = string_cast(cells[2]->Value->ToString());
				entry["Price"] = Convert::ToInt16(cells[3]->Value->ToString());
			}
		}

		System::Void saveToolStripMenuItem_Click(System::Object^, System::EventArgs^) {
			Globals::save();
			saveTable(dataGridView1, Globals::mainDataJson);
			FolderBrowserDialog^ FBD = gcnew FolderBrowserDialog;
			if(FBD->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
				const auto path = string_cast(Path::Combine(FBD->SelectedPath, string_cast(mainDataFilename)));
				std::ofstream(path) << Globals::mainDataJson;
			}
		}

		void loadTable(const nlohmann::json& from, DataGridView^ to) {
			const auto& data = from["Data"];
			for(auto& entry : data) {
				auto date = DateTime::ParseExact(string_cast(entry["Date"].get<std::string>()), customFormat, nullptr);
				...
				to->Rows->Add(date.ToString(customFormat), name, services, price);
			}
		}
		...
		System::Void loadToolStripMenuItem_Click(System::Object^, System::EventArgs^) {
			...
			if(OFD->ShowDialog() == Windows::Forms::DialogResult::OK)
				std::ifstream(string_cast(OFD->FileName)) >> Globals::mainDataJson;
			loadTable(Globals::mainDataJson, dataGridView1);
			sort();
		}
		...
		System::Void button1_Click(System::Object^, System::EventArgs^) {
			...
			auto DTV = dateTimePicker1->Value;
			dataGridView1->Rows->Add(DTV.ToString(customFormat), customerNametb->Text, selectedItems, totalPrice);
		...
		inline void updateUI() {
			nlohmann::json temp;
			saveTable(dataGridView1, temp);

			this->Controls->Clear();
			InitializeComponent();
			Globals::applyTheme(this);

			loadTable(temp, dataGridView1);
		}
